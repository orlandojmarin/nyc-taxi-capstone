...
Joins `stg_trips` (filtered to `IS_VALID = TRUE`) with `stg_zones` (for pickup and dropoff borough/zone names) and `stg_weather` (on date + hour for weather conditions at time of pickup). Carries through all revenue columns: fare_amount, extra, mta_tax, tip_amount, tolls_amount, improvement_surcharge, congestion_surcharge, cbd_congestion_fee, airport_fee, total_amount. Maps integer `payment_type` codes to human-readable labels (Credit Card, Cash, No Charge, Dispute, Unknown, Voided) for business-friendly output.
